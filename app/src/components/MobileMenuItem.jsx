...
const MobileMenuItem = ({ open, onClose = () => { } }) => {
  ...
  return (
    <section
      className={` fixed top-0 w-full h-screen duration-700  bg-slate-50/45 z-[999] font-outfit ${open ? "left-0" : "left-[-100%]"
        }`}
      onClick={onClose}
    >
      <article
      ...
      >
        ...
        {menuData.map((menuItem, idx) => {
          return (
            <div
              key={menuItem.key}
              className={`px-2 py-1 cursor-pointer rounded-xl flex  justify-center items-center gap-1 duration-300 transition-all text-start w-[80%]  ${location.pathname === menuItem.path && menuActiveBg
                } `}
              onClick={() => {
                onClose();
                setTimeout(() => {
        ...
        })}
        {
          ...
          >
            <Link
              key={"signin"}
              className={`   ${location.pathname === "/signin" && menuActiveBg
                } `}
              to={"/signin"}
            >
              Signin
